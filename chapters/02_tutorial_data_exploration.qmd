...
PCA has successfully accomplished a statistical task: it has represented correlated variation among several measured variables using fewer dimensions. However, what it has **not** established is that those new dimensions represent the causal quantities required by our estimand. Before using them in an estimator, we would need to ask what scientific quantities they represent and whether replacing the original variables preserves the causal roles encoded in our DAG. You can clearly see how the use of Principal Components Analysis has added a level of complication to our workflow.
