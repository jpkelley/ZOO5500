...
#### _Proxy or index variables_

The last scenario is when the quantity that you measure is not the process itself. Rather, it is a local proxy for a broader-scale, unmeasured cause. Consider a system where $territory\_quality$ is an unmeasured (*latent*) variable operating at a broader scale, while $canopy\_cover$ is a directly observed, local measurement. Here is the DAG:
...
Admittedly, this is a tricky one. But it is important, especially given that ecologists deal with things like commonly used indices (that may have historical relevance) or surveys that involve estimation of detectability. 
