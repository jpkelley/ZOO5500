---
title: "DECOMP_part2_causal_decomposition_from_parent_sets"
format: html
editor_options: 
...
\[
X_i \sim \text{Gaussian}\left(\mu_{X,i}, \sigma_X^2\right)
\]

\[
\mu_{X,i} = f_X(C_i) + b^{(X)}_{\text{site}[i]} + b^{(X)}_{\text{obs}[i]}
\]

\[
M1_i \sim \text{Gaussian}\left(\mu_{M1,i}, \sigma_{M1}^2\right)
\]

\[
\mu_{M1,i} = f_{M1,1}(X_i) + f_{M1,2}(C_i) + b^{(M1)}_{\text{site}[i]} + b^{(M1)}_{\text{obs}[i]}
\]

\[
M2_i \sim \text{Gaussian}\left(\mu_{M2,i}, \sigma_{M2}^2\right)
\]

\[
\mu_{M2,i} = f_{M2,1}(X_i) + f_{M2,2}(M1_i) + b^{(M2)}_{\text{site}[i]} + b^{(M2)}_{\text{obs}[i]}
\]

\[
Y_i \sim \text{scaled-}t\left(\mu_{Y,i}, \sigma_Y, \nu\right)
\]

\[
\mu_{Y,i} = f_{Y,1}(X_i) + f_{Y,2}(M2_i) + f_{Y,3}(C_i) + b^{(Y)}_{\text{site}[i]} + b^{(Y)}_{\text{obs}[i]}
\]
