...
Count distributions --often best approximated by a Poisson distribution-- have their own features, including:
...
A Gaussian model ignores almost everything about how most ecological data are generated. Therefore, in this section, we will first explore the Poisson GLM, which is designed specifically for count data. It:
...
First, we begin with our observations (our dataset). Using each observation $i$ and sample size $n$, we define our dataset as:
...
The equation prevents hidden assumptions. Without it, readers might assume constant variance or continuous outcomes. The formal notation makes those assumptions transparent. 

These points will be reiterated below in the link function section.
...
The final component of our GLM is the link function, which allows us to answer a simple and necessary question:

> How does the linear predictor (signal) determine the mean of the response distribution?

For GLMs, the mean of the response distribution directly determines the error structure because the variance is defined as some function of the mean. Let us look at its form in the case of the Poisson GLM. Recall from the previous page that, for a Poisson model:
...
> The expected value (long-run average) of the response $Y_i$ equals $\mu_i$.

More simply:

* $Y_i$ is the observed outcome for observation $i$.
* $\mu_i$ is the average value we would expect if we could repeat the same process many times under identical conditions.
* $E(Y_i)$ is simply statistical language for “mean.”

So we interpret this equation as:

> If we repeatedly generated data under the same conditions for observation $i$, the average of those outcomes would be $\mu_i$.

That hopefully makes a bit more sense!

:::

Because the variance equals the mean in a Poisson model, once we determine $\mu_i$, we automatically determine the variance as well. So what does the link function actually do?

We first build a straight-line equation:
...
That is all the link function is: a rule that converts the output of the linear predictor into a proper mean for the distribution. Since the variance depends on the mean, this transformation also determines how much noise we expect. In short, the following is happening:

* predictors determine $\eta_i$
* the link function converts $\eta_i$ into the mean $\mu_i$
* The mean determines the variance

The link is simply the bridge between signal and noise.
...
For this conceptual self-test, we will use a dataset that was collected from a population of *Lepusantilocapra pufferi* (puffer jackalope) starting in the mid-1970s. The data include the following variables:
