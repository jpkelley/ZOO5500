...
## A preliminary analysis using a GLM

### Start with a misspecified Generalized Linear Model (GLM)

As before, we first describe the model for testing the impact of certain predictors on the response variable (number of parasites).

We have several hypotheses to test. These hypotheses can easily be verbalized in a way that reflects how we might test them with GLMs, which we have already seen before. That is, we think about a few of the variables and hypothesize that the number of jackalopes is impacted by:
...
You have used GLMs to test eight competing (i.e. multiple working) hypotheses. Let us look at the the first model you ran.
...
The third model (`mod_03`) has the the lowest AIC value and therefore our best-supported model among those considered in the model set. We will discuss conventional approaches to AIC-based model selection later.
